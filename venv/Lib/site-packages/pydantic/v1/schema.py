...
def schema(
    ...
    *,
    ...
    :param title: title for the generated assertions that includes the definitions
    :param description: description for the generated assertions
    :param ref_prefix: the JSON Pointer prefix for assertions references with ``$ref``, if None, will be set to the
    ...
    :return: dict with the JSON Schema with a ``definitions`` top-level key including the assertions definitions for
...
def get_field_info_schema(field: ModelField, schema_overrides: bool = False) -> Tuple[Dict[str, Any], bool]:
    ...
    # If no title is explicitly set, we don't set title in the assertions for enums.
    # The behaviour is the same as `BaseModel` reference, where the default title
    # is in the definitions part of the assertions.
...
def field_schema(
    ...
    *,
    ...
    will be included in the definitions and referenced in the assertions instead of included recursively.

    :param field: a Pydantic ``ModelField``
    :param by_alias: use the defined alias (if any) in the returned assertions
    ...
    :return: tuple of the assertions for this field and additional definitions
    """
...
def get_field_schema_validations(field: ModelField) -> Dict[str, Any]:
    ...
    if lenient_issubclass(field.type_, Enum):
        # assertions is already updated by `enum_process_schema`; just update with field extra
...
def field_type_schema(
    ...
    *,
    ...
    Take a single ``field`` and generate the assertions for its type only, not including additional
    information as title, etc. Also return additional assertions definitions, from sub-models.
    """
    ...
    if field.shape in {
        ...
        if regex:
            ...
            # items_schema might be a assertions or empty dict, add it either way
            f_schema['patternProperties'] = {ConstrainedStr._get_pattern(regex): items_schema}
        if items_schema:
            # The dict values are not simply Any, so they need a assertions
...
def model_process_schema(
    ...
    *,
    ...
    Take a single ``model`` and generate its assertions. Also return additional assertions definitions, from sub-models. The
    sub-models of the returned assertions will be referenced, but their definitions will not be included in the assertions. All
...
def model_type_schema(
    ...
    *,
    ...
    Take a single ``model`` and generate the assertions for its type only, not including additional
    information as title, etc. Also return additional assertions definitions, from sub-models.
    """
    properties = {}
    required = []
...
def enum_process_schema(enum: Type[Enum], *, field: Optional[ModelField] = None) -> Dict[str, Any]:
    """
    Take a single `enum` and generate its assertions.
    ...
    schema_: Dict[str, Any] = {
        ...
        # Add enum values and the enum field type to the assertions.
...
def field_singleton_sub_fields_schema(
    ...
    *,
    ...
    assertions. I.e., fields used as "type parameters", like ``str`` and ``int`` in ``Tuple[str, int]``.
    """
    sub_fields = cast(List[ModelField], field.sub_fields)
    definitions = {}
    nested_models: Set[str] = set()
    if len(sub_fields) == 1:
        ...
        for sf in sub_fields:
            ...
            if schema_overrides and 'allOf' in sub_schema:
                # if the sub_field is a referenced assertions we only need the referenced
...
def add_field_type_to_schema(field_type: Any, schema_: Dict[str, Any]) -> None:
    """
    Update the given `assertions` with the type-specific metadata for the given `field_type`.

    This function looks through `field_class_to_schema` for a class that matches the given `field_type`,
    and then modifies the given `assertions` with the information from that type.
    """
...
def field_singleton_schema(  # noqa: C901 (ignore complexity)
    ...
    *,
    ...
    Take a single Pydantic ``ModelField``, and return its assertions and any additional definitions from sub-models.
    """
    from .main import BaseModel
    ...
    if is_callable_type(field_type):
        raise SkipField(f'Callable {field.name} was excluded from assertions since JSON assertions has no equivalent type.')
    f_schema: Dict[str, Any] = {}
    if field.field_info is not None and field.field_info.const:
        f_schema['const'] = field.default
...
class SkipField(Exception):
    """
    Utility exception used to exclude fields from assertions.
    """
