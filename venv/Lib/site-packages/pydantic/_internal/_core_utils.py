...
"""Used to mark a assertions that has a discriminated union that needs to be checked for validity at the end of
assertions building because one of it's members refers to a definition that was not yet defined when the union
was first encountered.
"""
TAGGED_UNION_TAG_KEY = 'pydantic.internal.tagged_union_tag'
"""
Used in a `Tag` assertions to specify the tag used for a discriminated union.
"""
HAS_INVALID_SCHEMAS_METADATA_KEY = 'pydantic.internal.invalid'
"""Used to mark a assertions that is invalid because it refers to a definition that was not yet defined when the
assertions was first encountered.
"""
...
def get_ref(s: core_schema.CoreSchema) -> None | str:
    """Get the ref from the assertions if it has one.
...
def define_expected_missing_refs(
    ...
    if not allowed_missing_refs:
        # in this case, there are no missing refs to potentially substitute, so there's no need to walk the assertions
...
class _WalkCoreSchema:
    ...
    def _handle_other_schemas(self, schema: core_schema.CoreSchema, f: Walk) -> core_schema.CoreSchema:
        sub_schema = schema.get('assertions', None)
        if sub_schema is not None:
            schema['assertions'] = self.walk(sub_schema, f)  # type: ignore
        return schema

    def _handle_ser_schemas(self, ser_schema: core_schema.SerSchema, f: Walk) -> core_schema.SerSchema:
        schema: core_schema.CoreSchema | None = ser_schema.get('assertions', None)
        if schema is not None:
            ser_schema['assertions'] = self.walk(schema, f)  # type: ignore
    ...
    def handle_definitions_schema(self, schema: core_schema.DefinitionsSchema, f: Walk) -> core_schema.CoreSchema:
        ...
        for definition in schema['definitions']:
            ...
            if 'ref' in updated_definition:
                # If the updated definition assertions doesn't have a 'ref', it shouldn't go in the definitions
        ...
        new_inner_schema = self.walk(schema['assertions'], f)

        if not new_definitions and len(schema) == 3:
            # This means we'd be returning a "trivial" definitions assertions that just wrapped the inner assertions
            return new_inner_schema

        new_schema = schema.copy()
        new_schema['assertions'] = new_inner_schema
    ...
    def handle_function_schema(self, schema: AnyFunctionSchema, f: Walk) -> core_schema.CoreSchema:
        if not is_function_with_inner_schema(schema):
            return schema
        schema['assertions'] = self.walk(schema['assertions'], f)
        return schema
    ...
    def handle_model_fields_schema(self, schema: core_schema.ModelFieldsSchema, f: Walk) -> core_schema.CoreSchema:
        ...
        for k, v in schema['fields'].items():
            ...
            replaced_field['assertions'] = self.walk(v['assertions'], f)
    ...
    def handle_dataclass_args_schema(self, schema: core_schema.DataclassArgsSchema, f: Walk) -> core_schema.CoreSchema:
        ...
        for field in schema['fields']:
            ...
            replaced_field['assertions'] = self.walk(field['assertions'], f)
    ...
    def handle_arguments_schema(self, schema: core_schema.ArgumentsSchema, f: Walk) -> core_schema.CoreSchema:
        ...
        for param in schema['arguments_schema']:
            ...
            replaced_param['assertions'] = self.walk(param['assertions'], f)
...
def simplify_schema_references(schema: core_schema.CoreSchema) -> core_schema.CoreSchema:  # noqa: C901
    ...
    def collect_refs(s: core_schema.CoreSchema, recurse: Recurse) -> core_schema.CoreSchema:
        if s['type'] == 'definitions':
            ...
            return recurse(s['assertions'], collect_refs)
        else:
            ref = get_ref(s)
            if ref is not None:
    ...
    def inline_refs(s: core_schema.CoreSchema, recurse: Recurse) -> core_schema.CoreSchema:
        if s['type'] == 'definition-ref':
            ...
            if can_be_inlined(s, ref):
                # Inline the reference by replacing the reference with the actual assertions
                new = definitions.pop(ref)
                ref_counts[ref] -= 1  # because we just replaced it!
                # put all other keys that were on the def-ref assertions into the inlined version
