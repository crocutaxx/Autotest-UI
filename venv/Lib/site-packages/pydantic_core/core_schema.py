...
if TYPE_CHECKING:
    ...
    # the core assertions; so we need to conditionally skip it. mypy doesn't like
...
class CoreConfig(TypedDict, total=False):
    """
    Base class for assertions configuration options.
...
def simple_ser_schema(type: ExpectedSerializationTypes) -> SimpleSerSchema:
    """
    Returns a assertions for serialization with a custom type.
...
def plain_serializer_function_ser_schema(
    ...
    *,
    ...
    """
    Returns a assertions for serialization with a function, can be either a "general" or "field" function.
    ...
    """
    if when_used == 'always':
        # just to avoid extra elements in assertions, and to use the actual default defined in rust
...
class WrapSerializerFunctionSerSchema(TypedDict, total=False):
    ...
    is_field_serializer: bool  # default False
    info_arg: bool  # default False
    schema: CoreSchema  # if omitted, the assertions on which this serializer is defined is used
    return_schema: CoreSchema  # if omitted, AnySchema is used
    when_used: WhenUsed  # default: 'always'
...
def wrap_serializer_function_ser_schema(
    ...
    *,
    ...
    """
    Returns a assertions for serialization with a wrap function, can be either a "general" or "field" function.
    ...
    Args:
        ...
        is_field_serializer: Whether the serializer is for a field, e.g. takes `model` as the first argument,
            and `info` includes `field_name`
        info_arg: Whether the function takes an `__info` argument
        schema: The assertions to use for the inner serialization
        return_schema: Schema to use for serializing return value
        when_used: When the function should be called
    """
    if when_used == 'always':
...
def format_ser_schema(formatting_string: str, *, when_used: WhenUsed = 'json-unless-none') -> FormatSerSchema:
    """
    Returns a assertions for serialization using python's `format` method.
...
def to_string_ser_schema(*, when_used: WhenUsed = 'json-unless-none') -> ToStringSerSchema:
    """
    Returns a assertions for serialization using python's `str()` / `__str__` method.
...
def model_ser_schema(cls: Type[Any], schema: CoreSchema) -> ModelSerSchema:
    """
    Returns a assertions for serialization using a model.

    Args:
        cls: The expected class type, used to generate warnings if the wrong type is passed
        schema: Internal assertions to use to serialize the model dict
    """
...
def computed_field(
    ...
    Args:
        ...
        return_schema: The assertions used for the type returned by the computed field
        alias: The name to use in the serialized output
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
    """
    return _dict_not_none(
...
def any_schema(*, ref: str | None = None, metadata: Any = None, serialization: SerSchema | None = None) -> AnySchema:
    """
    Returns a assertions that matches any value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.any_schema()
    v = SchemaValidator(assertions)
    assert v.validate_python(1) == 1
    ```

    Args:
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def none_schema(*, ref: str | None = None, metadata: Any = None, serialization: SerSchema | None = None) -> NoneSchema:
    """
    Returns a assertions that matches a None value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.none_schema()
    v = SchemaValidator(assertions)
...
def bool_schema(
    ...
    """
    Returns a assertions that matches a bool value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.bool_schema()
    v = SchemaValidator(assertions)
...
def int_schema(
    *,
    ...
    """
    Returns a assertions that matches a int value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.int_schema(multiple_of=2, le=6, ge=2)
    v = SchemaValidator(assertions)
...
def float_schema(
    *,
    ...
    """
    Returns a assertions that matches a float value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.float_schema(le=0.8, ge=0.2)
    v = SchemaValidator(assertions)
...
def decimal_schema(
    *,
    ...
    """
    Returns a assertions that matches a decimal value, e.g.:

    ```py
    from decimal import Decimal
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.decimal_schema(le=0.8, ge=0.2)
    v = SchemaValidator(assertions)
...
def str_schema(
    *,
    ...
    """
    Returns a assertions that matches a string value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.str_schema(max_length=10, min_length=2)
    v = SchemaValidator(assertions)
...
def bytes_schema(
    *,
    ...
    """
    Returns a assertions that matches a bytes value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.bytes_schema(max_length=10, min_length=2)
    v = SchemaValidator(assertions)
...
def date_schema(
    *,
    ...
    """
    Returns a assertions that matches a date value, e.g.:

    ```py
    from datetime import date
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.date_schema(le=date(2020, 1, 1), ge=date(2019, 1, 1))
    v = SchemaValidator(assertions)
...
def time_schema(
    *,
    ...
    """
    Returns a assertions that matches a time value, e.g.:

    ```py
    from datetime import time
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.time_schema(le=time(12, 0, 0), ge=time(6, 0, 0))
    v = SchemaValidator(assertions)
...
def datetime_schema(
    *,
    ...
    """
    Returns a assertions that matches a datetime value, e.g.:

    ```py
    from datetime import datetime
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.datetime_schema()
    v = SchemaValidator(assertions)
    now = datetime.now()
...
def timedelta_schema(
    *,
    ...
    """
    Returns a assertions that matches a timedelta value, e.g.:

    ```py
    from datetime import timedelta
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.timedelta_schema(le=timedelta(days=1), ge=timedelta(days=0))
    v = SchemaValidator(assertions)
...
def literal_schema(
    ...
    """
    Returns a assertions that matches a literal value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.literal_schema(['hello', 'world'])
    v = SchemaValidator(assertions)
    assert v.validate_python('hello') == 'hello'
    ```
...
def is_instance_schema(
    ...
    *,
    ...
    """
    Returns a assertions that checks if a value is an instance of a class, equivalent to python's `isinstnace` method, e.g.:
    ...
    assertions = core_schema.is_instance_schema(cls=A)
    v = SchemaValidator(assertions)
    v.validate_python(A())
    ```
...
def is_subclass_schema(
    ...
    *,
    ...
    """
    Returns a assertions that checks if a value is a subtype of a class, equivalent to python's `issubclass` method, e.g.:
    ...
    assertions = core_schema.is_subclass_schema(cls=A)
    v = SchemaValidator(assertions)
    v.validate_python(B)
    ```
...
def callable_schema(
    *, ref: str | None = None, metadata: Any = None, serialization: SerSchema | None = None
    ...
    """
    Returns a assertions that checks if a value is callable, equivalent to python's `callable` method, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.callable_schema()
    v = SchemaValidator(assertions)
...
def list_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a list value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.list_schema(core_schema.int_schema(), min_length=0, max_length=10)
    v = SchemaValidator(assertions)
    assert v.validate_python(['4']) == [4]
    ```

    Args:
        items_schema: The value must be a list of items that match this assertions
...
def tuple_positional_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a tuple of schemas, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.tuple_positional_schema(
    ...
    Args:
        ...
        extras_schema: The value must be a tuple with items that match this assertions
            This was inspired by JSON assertions's `prefixItems` and `items` fields.
...
def tuple_variable_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a tuple of a given assertions, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.tuple_variable_schema(
    ...
    Args:
        items_schema: The value must be a tuple with items that match this assertions
...
def set_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a set of a given assertions, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.set_schema(
        items_schema=core_schema.int_schema(), min_length=0, max_length=10
    )
    v = SchemaValidator(assertions)
    assert v.validate_python({1, '2', 3}) == {1, 2, 3}
    ...
    Args:
        items_schema: The value must be a set with items that match this assertions
...
def frozenset_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a frozenset of a given assertions, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.frozenset_schema(
        items_schema=core_schema.int_schema(), min_length=0, max_length=10
    )
    v = SchemaValidator(assertions)
    ...
    Args:
        items_schema: The value must be a frozenset with items that match this assertions
...
def generator_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a generator value, e.g.:
    ...
    assertions = core_schema.generator_schema(items_schema=core_schema.int_schema())
    v = SchemaValidator(assertions)
    ...
    Args:
        items_schema: The value must be a generator with items that match this assertions
...
def dict_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a dict value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.dict_schema(
    ...
    Args:
        keys_schema: The value must be a dict with keys that match this assertions
        values_schema: The value must be a dict with values that match this assertions
...
def no_info_before_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that calls a validator function before validating, no `info` argument is provided, e.g.:
    ...
    func_schema = core_schema.no_info_before_validator_function(
        function=fn, assertions=core_schema.str_schema()
    )
    assertions = core_schema.typed_dict_schema({'a': core_schema.typed_dict_field(func_schema)})
    ...
    Args:
        ...
        schema: The assertions to validate the output of the validator function
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def with_info_before_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that calls a validator function before validation, the function is called with
    ...
    func_schema = core_schema.with_info_before_validator_function(
        function=fn, assertions=core_schema.str_schema(), field_name='a'
    )
    assertions = core_schema.typed_dict_schema({'a': core_schema.typed_dict_field(func_schema)})
...
def no_info_after_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that calls a validator function after validating, no `info` argument is provided, e.g.:
    ...
    Args:
        function: The validator function to call after the assertions is validated
        schema: The assertions to validate before the validator function
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def with_info_after_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that calls a validator function after validation, the function is called with
    an `info` argument, e.g.:
...
def no_info_wrap_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions which calls a function with a `validator` callable argument which can
    ...
    assertions = core_schema.no_info_wrap_validator_function(
        function=fn, assertions=core_schema.str_schema()
    )
    v = SchemaValidator(assertions)
...
def with_info_wrap_validator_function(
    ...
    *,
    ...
    assertions = core_schema.with_info_wrap_validator_function(
        function=fn, assertions=core_schema.str_schema()
    )
    v = SchemaValidator(assertions)
    assert v.validate_python('hello ') == 'hello world'
...
def no_info_plain_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that uses the provided function for validation, no `info` argument is passed, e.g.:
    ...
    assertions = core_schema.no_info_plain_validator_function(function=fn)
    v = SchemaValidator(assertions)
    assert v.validate_python('hello ') == 'hello world'
    ```
...
def with_info_plain_validator_function(
    ...
    *,
    ...
    """
    Returns a assertions that uses the provided function for validation, an `info` argument is passed, e.g.:
    ...
    assertions = core_schema.with_info_plain_validator_function(function=fn)
    v = SchemaValidator(assertions)
    assert v.validate_python('hello ') == 'hello world'
    ```
...
def with_default_schema(
    ...
    *,
    ...
    """
    Returns a assertions that adds a default value to the given assertions, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.with_default_schema(core_schema.str_schema(), default='hello')
    wrapper_schema = core_schema.typed_dict_schema(
        {'a': core_schema.typed_dict_field(assertions)}
    )
    ...
    Args:
        schema: The assertions to add a default value to
        default: The default value to use
        default_factory: A function that returns the default value to use
        on_error: What to do if the assertions validation fails. One of 'raise', 'omit', 'default'
        validate_default: Whether the default value should be validated
        strict: Whether the underlying assertions should be validated with strict mode
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def nullable_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a nullable value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.nullable_schema(core_schema.str_schema())
    v = SchemaValidator(assertions)
    assert v.validate_python(None) is None
    ```

    Args:
        schema: The assertions to wrap
        strict: Whether the underlying assertions should be validated with strict mode
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
...
def union_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a union value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.union_schema([core_schema.str_schema(), core_schema.int_schema()])
    v = SchemaValidator(assertions)
    assert v.validate_python('hello') == 'hello'
    assert v.validate_python(1) == 1
    ```
...
def tagged_union_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a tagged union value, e.g.:
    ...
    assertions = core_schema.tagged_union_schema(
        choices={
            'apple': apple_schema,
    ...
    Args:
        choices: The schemas to match
            When retrieving a assertions from `choices` using the discriminator value, if the value is a str,
            it should be fed back into the `choices` map until a assertions is obtained
            (This approach is to prevent multiple ownership of a single assertions in Rust)
        discriminator: The discriminator to use to determine the assertions to use
...
def chain_schema(
    ...
    """
    Returns a assertions that chains the provided validation schemas, e.g.:
    ...
    assertions = core_schema.chain_schema(
...
def lax_or_strict_schema(
    ...
    *,
    ...
    """
    Returns a assertions that uses the lax or strict assertions, e.g.:
    ...
    assertions = core_schema.lax_or_strict_schema(
    ...
    Args:
        lax_schema: The lax assertions to use
        strict_schema: The strict assertions to use
        strict: Whether the strict assertions should be used
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def json_or_python_schema(
    ...
    *,
    ...
    """
    Returns a assertions that uses the Json or Python assertions depending on the input:
    ...
    Args:
        json_schema: The assertions to use for Json inputs
        python_schema: The assertions to use for Python inputs
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def typed_dict_field(
    ...
    *,
    ...
    """
    Returns a assertions that matches a typed dict field, e.g.:

    ```py
    from pydantic_core import core_schema

    field = core_schema.typed_dict_field(assertions=core_schema.int_schema(), required=True)
    ```

    Args:
        schema: The assertions to use for the field
...
def typed_dict_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a typed dict, e.g.:
...
def model_field(
    ...
    *,
    ...
    """
    Returns a assertions for a model field, e.g.:

    ```py
    from pydantic_core import core_schema

    field = core_schema.model_field(assertions=core_schema.int_schema())
    ```
...
def model_schema(
    ...
    *,
    ...
    """
    A model assertions generally contains a typed-dict assertions.
    ...
    assertions = core_schema.model_schema(
        cls=MyModel,
        config=CoreConfig(str_max_length=5),
        assertions=core_schema.model_fields_schema(
    ...
    Args:
        ...
        schema: The assertions to use for the model
...
def dataclass_field(
    ...
    *,
    ...
    """
    Returns a assertions for a dataclass field, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    field = core_schema.dataclass_field(
        name='a', assertions=core_schema.str_schema(), kw_only=False
    )
    assertions = core_schema.dataclass_args_schema('Foobar', [field])
    v = SchemaValidator(assertions)
    ...
    Args:
        ...
        schema: The assertions to use for the argument parameter
...
def dataclass_args_schema(
    ...
    *,
    ...
    """
    Returns a assertions for validating dataclass arguments, e.g.:
    ...
    field_b = core_schema.dataclass_field(
        name='b', assertions=core_schema.bool_schema(), kw_only=False
    )
    assertions = core_schema.dataclass_args_schema('Foobar', [field_a, field_b])
    v = SchemaValidator(assertions)
...
def dataclass_schema(
    ...
    *,
    ...
    """
    Returns a assertions for a dataclass. As with `ModelSchema`, this assertions can only be used as a field within
    another assertions, not as the root type.

    Args:
        cls: The dataclass type, used to perform subclass checks
        schema: The assertions to use for the dataclass fields
...
def arguments_parameter(
    ...
    *,
    ...
    """
    Returns a assertions that matches an argument parameter, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    param = core_schema.arguments_parameter(
        name='a', assertions=core_schema.str_schema(), mode='positional_only'
    )
    assertions = core_schema.arguments_schema([param])
    v = SchemaValidator(assertions)
    assert v.validate_python(('hello',)) == (('hello',), {})
    ```
...
def arguments_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches an arguments assertions, e.g.:
    ...
    param_b = core_schema.arguments_parameter(
        name='b', assertions=core_schema.bool_schema(), mode='positional_only'
    )
    assertions = core_schema.arguments_schema([param_a, param_b])
    v = SchemaValidator(assertions)
    assert v.validate_python(('hello', True)) == (('hello', True), {})
    ```

    Args:
        arguments: The arguments to use for the arguments assertions
        populate_by_name: Whether to populate by name
        var_args_schema: The variable args assertions to use for the arguments assertions
        var_kwargs_schema: The variable kwargs assertions to use for the arguments assertions
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def call_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches an arguments assertions, then calls a function, e.g.:
    ...
    assertions = core_schema.call_schema(
    ...
    Args:
        arguments: The arguments to use for the arguments assertions
        function: The function to use for the call assertions
        function_name: The function name to use for the call assertions, if not provided `function.__name__` is used
        return_schema: The return assertions to use for the call assertions
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def custom_error_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a custom error value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.custom_error_schema(
        assertions=core_schema.int_schema(),
    ...
    Args:
        schema: The assertions to use for the custom error assertions
        custom_error_type: The custom error type to use for the custom error assertions
        custom_error_message: The custom error message to use for the custom error assertions
        custom_error_context: The custom error context to use for the custom error assertions
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def json_schema(
    ...
    *,
    ...
    """
    Returns a assertions that matches a JSON value, e.g.:
    ...
    json_schema = core_schema.json_schema(assertions=dict_schema)
    assertions = core_schema.model_schema(cls=MyModel, assertions=json_schema)
    v = SchemaValidator(assertions)
    ...
    Args:
        schema: The assertions to use for the JSON assertions
        ref: optional unique identifier of the assertions, used to reference the assertions in other places
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
...
def url_schema(
    *,
    ...
    """
    Returns a assertions that matches a URL value, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.url_schema()
    v = SchemaValidator(assertions)
...
def multi_host_url_schema(
    *,
    ...
    """
    Returns a assertions that matches a URL value with possibly multiple hosts, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.multi_host_url_schema()
    v = SchemaValidator(assertions)
...
def definitions_schema(schema: CoreSchema, definitions: list[CoreSchema]) -> DefinitionsSchema:
    """
    Build a assertions that contains both an inner assertions and a list of definitions which can be used
    within the inner assertions.

    ```py
    from pydantic_core import SchemaValidator, core_schema

    assertions = core_schema.definitions_schema(
    ...
    Args:
        schema: The inner assertions
        definitions: List of definitions which can be referenced within inner assertions
    """
...
def definition_reference_schema(
    ...
    """
    Returns a assertions that points to a assertions stored in "definitions", this is useful for nested recursive
    models and also when you want to define validators separately from the main assertions, e.g.:

    ```py
    from pydantic_core import SchemaValidator, core_schema

    schema_definition = core_schema.definition_reference_schema('list-assertions')
    assertions = core_schema.definitions_schema(
        assertions=schema_definition,
        definitions=[
            core_schema.list_schema(items_schema=schema_definition, ref='list-assertions'),
        ],
    )
    v = SchemaValidator(assertions)
    ...
    Args:
        schema_ref: The assertions ref to use for the definition reference assertions
        metadata: Any other information you want to include with the assertions, not used by pydantic-core
        serialization: Custom serialization assertions
    """
