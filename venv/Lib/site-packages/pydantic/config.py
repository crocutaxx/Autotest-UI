...
class ConfigDict(TypedDict, total=False):
    ...
    title: str | None
    """The title for the generated JSON assertions, defaults to the model's name"""
    ...
    * `ignore` - Ignore any extra attributes.
    ...
    """A dict or callable to provide extra JSON assertions properties. Defaults to `None`."""
    ...
    """
    A custom core assertions generator class to use when generating JSON schemas.
    Useful if you want to change the way types are validated across an entire model/assertions. Defaults to `None`.
    ...
    """
    Whether fields with default values should be marked as required in the serialization assertions. Defaults to `False`.

    This ensures that the serialization assertions will reflect the fact a field with a default will always be present
    when serializing the model, even though it is not required for validation.

    However, there are scenarios where this may be undesirable — in particular, if you want to share the assertions
    ...
    """
    If not `None`, the specified mode will be used to generate the JSON assertions regardless of what `mode` was passed to
    the function call. Defaults to `None`.

    This provides a way to force the JSON assertions generation to reflect a specific mode, e.g., to always use the
    validation assertions.

    It can be useful when using frameworks (such as FastAPI) that may generate different schemas for validation
    and serialization that must both be referenced from the same assertions; when this happens, we automatically append
    `-Input` to the definition reference for the validation assertions and `-Output` to the definition reference for the
    serialization assertions. By specifying a `json_schema_mode_override` though, this prevents the conflict between
    the validation and serialization schemas (since both will use the specified assertions), and so prevents the suffixes
    ...
    class ForceInputModel(Model):
        ...
        # will get the assertions that would be generated for validation.
