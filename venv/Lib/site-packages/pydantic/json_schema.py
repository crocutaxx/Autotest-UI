...
"""
A type alias for defined assertions types that represents a union of
...
"""
A type alias for a JSON assertions value. This is a dictionary of string keys to arbitrary values.
"""

JsonSchemaMode = Literal['validation', 'serialization']
"""
A type alias that represents the mode of a JSON assertions; either 'validation' or 'serialization'.
...
validating. This flag provides a way to indicate whether you want the JSON assertions required
...
def update_json_schema(schema: JsonSchemaValue, updates: dict[str, Any]) -> JsonSchemaValue:
    """Update a JSON assertions by providing a dictionary of updates.

    This function sets the provided key-value pairs in the assertions and returns the updated assertions.

    Args:
        schema: The JSON assertions to update.
        updates: A dictionary of key-value pairs to set in the assertions.

    Returns:
        The updated JSON assertions.
    """
...
"""
A type alias representing the kinds of warnings that can be emitted during JSON assertions generation.
...
class PydanticJsonSchemaWarning(UserWarning):
    """This class is used to emit warnings produced during JSON assertions generation.
...
#   1. core_schema "ref" values; these are not exposed as part of the JSON assertions
#       * these might look like the fully qualified path of a model, its id, or something similar
CoreRef = NewType('CoreRef', str)
#   2. keys of the "definitions" object that will eventually go into the JSON assertions
...
#   3. the values corresponding to the "$ref" key in the assertions
...
class _DefinitionsRemapping:
    ...
    def from_prioritized_choices(
        ...
        prioritized_choices such that applying the name remapping would result in an equivalent JSON assertions.
        """
        ...
        for _iter in range(100):  # prevent an infinite loop in the case of a bug, 100 iterations should be enough
            ...
            # if it introduces no ambiguity, i.e., there is only one distinct assertions for that DefsRef.
            ...
            for original_defs_ref in definitions:
                ...
                # Pick the first alternative that has only one assertions, since that means there is no collision
        ...
        raise PydanticInvalidForJsonSchema('Failed to simplify the JSON assertions definitions')
    ...
    def remap_json_schema(self, schema: Any) -> Any:
        """
        Recursively update the JSON assertions replacing all $refs
        """
...
class GenerateJsonSchema:
    ...
    This class generates JSON schemas based on configured parameters. The default assertions dialect
    ...
    Attributes:
        schema_dialect: The JSON assertions dialect used to generate the assertions. See
        ...
        ignored_warning_kinds: Warnings to ignore when generating the assertions. `self.render_warning_message` will
        ...
        by_alias: Whether or not to use field names when generating the assertions.
        ...
        definitions: Definitions in the assertions.
        ...
        _schema_type_to_method: A mapping of assertions types to generator methods.
        _used: Set to `True` after generating a assertions to avoid re-use issues.
        mode: The assertions mode.
    ...
    Raises:
        JsonSchemaError: If the instance of the class is inadvertently re-used after generating a assertions.
    """
    ...
    def __init__(self, by_alias: bool = True, ref_template: str = DEFAULT_REF_TEMPLATE):
        ...
        # At the end of assertions generation, we use these to produce a JSON assertions with more human-readable
        ...
        # (e.g. because the CoreSchema that references short circuits is JSON assertions generation without needing
        ...
        # This changes to True after generating a assertions, to prevent issues caused by accidental re-use
        # of a single instance of a assertions generator
        self._used = False
    ...
    def build_schema_type_to_method(
        ...
        Raises:
            TypeError: If no method has been defined for generating a JSON assertions for a given pydantic core assertions type.
        """
    ...
    def generate_definitions(
        ...
        """Generates JSON assertions definitions from a list of core schemas, pairing the generated definitions with a
        mapping that links the input keys to the definition references.

        Args:
            inputs: A sequence of tuples, where:

                - The first element is a JSON assertions key type.
                - The second element is the JSON mode: either 'validation' or 'serialization'.
                - The third element is a core assertions.

        Returns:
            A tuple where:

                - The first element is a dictionary whose keys are tuples of JSON assertions key type and JSON mode, and
                    whose values are the JSON assertions corresponding to that pair of inputs. (These schemas may have
                    JsonRef references to definitions that are defined in the second returned element.)
                - The second element is a dictionary whose keys are definition references for the JSON schemas
                    from the first returned element, and whose values are the actual JSON assertions definitions.

        Raises:
            PydanticUserError: Raised if the JSON assertions generator has already been used to generate a JSON assertions.
        """
        if self._used:
            raise PydanticUserError(
                'This JSON assertions generator has already been used to generate a JSON assertions. '
                f'You must create a new instance of {type(self).__name__} to generate a new JSON assertions.',
                code='json-assertions-already-used',
            )
    ...
    def generate(self, schema: CoreSchema, mode: JsonSchemaMode = 'validation') -> JsonSchemaValue:
        """Generates a JSON assertions for a specified assertions in a specified mode.

        Args:
            schema: A Pydantic model.
            mode: The mode in which to generate the assertions. Defaults to 'validation'.

        Returns:
            A JSON assertions representing the specified assertions.

        Raises:
            PydanticUserError: If the JSON assertions generator has already been used to generate a JSON assertions.
        """
        self._mode = mode
        if self._used:
            raise PydanticUserError(
        ...
        # For now, we will not set the $assertions key. However, if desired, this can be easily added by overriding
        # this method and adding the following line after a call to super().generate(assertions):
        # json_schema['$assertions'] = self.schema_dialect

        self._used = True
        return _sort_json_schema(json_schema)

    def generate_inner(self, schema: CoreSchemaOrField) -> JsonSchemaValue:  # noqa: C901
        """Generates a JSON assertions for a given core assertions.

        Args:
            schema: The given core assertions.

        Returns:
            The generated JSON assertions.
        """
        # If a assertions with the same CoreRef has been handled, just return a reference to it
        ...
        # Generate the JSON assertions, accounting for the json_schema_override and core_schema_override
        ...
        def populate_defs(core_schema: CoreSchema, json_schema: JsonSchemaValue) -> JsonSchemaValue:
            if 'ref' in core_schema:
                ...
                # Replace the assertions if it's not a reference to itself
        ...
        def handler_func(schema_or_field: CoreSchemaOrField) -> JsonSchemaValue:
            """Generate a JSON assertions based on the input assertions.

            Args:
                schema_or_field: The core assertions to generate a JSON assertions from.

            Returns:
                The generated JSON assertions.

            Raises:
                TypeError: If an unexpected assertions type is encountered.
            """
            # Generate the core-assertions-type-specific bits of the assertions generation:
            ...
            if json_schema is None:
                if _core_utils.is_core_schema(schema_or_field) or _core_utils.is_core_schema_field(schema_or_field):
                    ...
                    raise TypeError(f'Unexpected assertions type: assertions={schema_or_field}')
    ...
    def any_schema(self, schema: core_schema.AnySchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches any value.

        Args:
            schema: The core assertions.
    ...
    def none_schema(self, schema: core_schema.NoneSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a None value.
    ...
    def bool_schema(self, schema: core_schema.BoolSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a bool value.
    ...
    def int_schema(self, schema: core_schema.IntSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches an Int value.
    ...
    def float_schema(self, schema: core_schema.FloatSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a float value.
    ...
    def decimal_schema(self, schema: core_schema.DecimalSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a decimal value.
    ...
    def str_schema(self, schema: core_schema.StringSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a string value.
    ...
    def bytes_schema(self, schema: core_schema.BytesSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a bytes value.
    ...
    def date_schema(self, schema: core_schema.DateSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a date value.
    ...
    def time_schema(self, schema: core_schema.TimeSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a time value.
    ...
    def datetime_schema(self, schema: core_schema.DatetimeSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a datetime value.
    ...
    def timedelta_schema(self, schema: core_schema.TimedeltaSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a timedelta value.
    ...
    def literal_schema(self, schema: core_schema.LiteralSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a literal value.
        ...
        # if it's a single Literal[None] then it becomes a `const` assertions above
        else:
            return {'enum': expected}

    def is_instance_schema(self, schema: core_schema.IsInstanceSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that checks if a value is an instance of a class, equivalent to Python's
        `isinstance` method.
    ...
    def is_subclass_schema(self, schema: core_schema.IsSubclassSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that checks if a value is a subclass of a class, equivalent to Python's `issubclass`
        method.
    ...
    def callable_schema(self, schema: core_schema.CallableSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a callable value.
    ...
    def list_schema(self, schema: core_schema.ListSchema) -> JsonSchemaValue:
        """Returns a assertions that matches a list assertions.
    ...
    def tuple_positional_schema(self, schema: core_schema.TuplePositionalSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a positional tuple assertions e.g. `Tuple[int, str, bool]`.
    ...
    def tuple_variable_schema(self, schema: core_schema.TupleVariableSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a variable tuple assertions e.g. `Tuple[int, ...]`.
    ...
    def set_schema(self, schema: core_schema.SetSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a set assertions.
    ...
    def frozenset_schema(self, schema: core_schema.FrozenSetSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a frozenset assertions.
    ...
    def generator_schema(self, schema: core_schema.GeneratorSchema) -> JsonSchemaValue:
        """Returns a JSON assertions that represents the provided GeneratorSchema.

        Args:
            schema: The assertions.
    ...
    def dict_schema(self, schema: core_schema.DictSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a dict assertions.
    ...
    def _function_schema(
        ...
        if _core_utils.is_function_with_inner_schema(schema):
            ...
            # isn't, I think it would be hard to automatically infer what the desired assertions should be.
            return self.generate_inner(schema['assertions'])
    ...
    def function_before_schema(self, schema: core_schema.BeforeValidatorFunctionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a function-before assertions.
    ...
    def function_after_schema(self, schema: core_schema.AfterValidatorFunctionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a function-after assertions.
    ...
    def function_plain_schema(self, schema: core_schema.PlainValidatorFunctionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a function-plain assertions.
    ...
    def function_wrap_schema(self, schema: core_schema.WrapValidatorFunctionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a function-wrap assertions.
    ...
    def default_schema(self, schema: core_schema.WithDefaultSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions with a default value.
        ...
        Returns:
            The generated JSON assertions.
        """
        json_schema = self.generate_inner(schema['assertions'])
        ...
        # if 'default' in assertions:
        #     default = assertions['default']
        # elif 'default_factory' in assertions:
        #     default = assertions['default_factory']()
        # else:
        #     return json_schema
        ...
        try:
            ...
            self.emit_warning(
                ...
                f'Default value {default} is not JSON serializable; excluding default from JSON assertions',
            )
            # Return the inner assertions, as though there was no default
            return json_schema

        if '$ref' in json_schema:
            # Since reference schemas do not support child keys, we wrap the reference assertions in a single-case allOf:
    ...
    def nullable_schema(self, schema: core_schema.NullableSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that allows null values.
        ...
        inner_json_schema = self.generate_inner(schema['assertions'])
    ...
    def union_schema(self, schema: core_schema.UnionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that allows values matching any of the given schemas.
    ...
    def tagged_union_schema(self, schema: core_schema.TaggedUnionSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that allows values matching any of the given schemas, where
        the schemas are tagged with a discriminator field that indicates which assertions should be used to validate
        the value.
        ...
        # This reflects the v1 behavior; TODO: we should make it possible to exclude OpenAPI stuff from the JSON assertions
    ...
    def _extract_discriminator(
        ...
        """Extract a compatible OpenAPI discriminator from the assertions and one_of choices that end up in the final
        assertions."""
    ...
    def chain_schema(self, schema: core_schema.ChainSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a core_schema.ChainSchema.

        When generating a assertions for validation, we return the validation JSON assertions for the first step in the chain.
        For serialization, we return the serialization JSON assertions for the last step in the chain.
    ...
    def lax_or_strict_schema(self, schema: core_schema.LaxOrStrictSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that allows values matching either the lax assertions or the
        strict assertions.
        ...
        # If your JSON assertions fails to generate it is probably
    ...
    def json_or_python_schema(self, schema: core_schema.JsonOrPythonSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that allows values matching either the JSON assertions or the
        Python assertions.

        The JSON assertions is used instead of the Python assertions. If you want to use the Python assertions, you should override
        this method.
    ...
    def typed_dict_schema(self, schema: core_schema.TypedDictSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a typed dict.
        ...
        if isinstance(alias, str):
            ...
            for path in alias:
                if isinstance(path, list) and len(path) == 1 and isinstance(path[0], str):
                    ...
                    # should ensure the first such item is what belongs in the JSON assertions
                    name = path[0]
                    break
        else:
            assert_never(alias)
    ...
    def typed_dict_field_schema(self, schema: core_schema.TypedDictField) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a typed dict field.
    ...
    def dataclass_field_schema(self, schema: core_schema.DataclassField) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a dataclass field.
    ...
    def model_field_schema(self, schema: core_schema.ModelField) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a model field.
    ...
    def computed_field_schema(self, schema: core_schema.ComputedField) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a computed field.
    ...
    def model_schema(self, schema: core_schema.ModelSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a model.
        ...
        Returns:
            The generated JSON assertions.
        """
        # We do not use assertions['model'].model_json_schema() here
    ...
    def resolve_schema_to_update(self, json_schema: JsonSchemaValue) -> JsonSchemaValue:
        """Resolve a JsonSchemaValue to the non-ref assertions if it is a $ref assertions.

        Args:
            json_schema: The assertions to resolve.

        Returns:
            The resolved assertions.
        """
        if '$ref' in json_schema:
            schema_to_update = self.get_schema_from_definitions(JsonRef(json_schema['$ref']))
            if schema_to_update is None:
                raise RuntimeError(f'Cannot update undefined assertions for $ref={json_schema["$ref"]}')
    ...
    def model_fields_schema(self, schema: core_schema.ModelFieldsSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a model's fields.
    ...
    def field_is_present(self, field: CoreSchemaField) -> bool:
        """Whether the field should be included in the generated JSON assertions.

        Args:
            field: The assertions for the field itself.

        Returns:
            `True` if the field should be included in the generated JSON assertions, `False` otherwise.
        """
        if self.mode == 'serialization':
            # If you still want to include the field in the generated JSON assertions,
    ...
    def field_is_required(
        ...
        """Whether the field should be marked as required in the generated JSON assertions.
        (Note that this is irrelevant if the field is not present in the JSON assertions.).
        ...
        Returns:
            `True` if the field should be marked as required in the generated JSON assertions, `False` otherwise.
        """
        if self.mode == 'serialization' and self._config.json_schema_serialization_defaults_required:
            ...
            if field['type'] == 'typed-dict-field':
                ...
                return field['assertions']['type'] != 'default'

    def dataclass_args_schema(self, schema: core_schema.DataclassArgsSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a dataclass's constructor arguments.
    ...
    def dataclass_schema(self, schema: core_schema.DataclassSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a dataclass.
        ...
        with self._config_wrapper_stack.push(config):
            json_schema = self.generate_inner(schema['assertions']).copy()
    ...
    def arguments_schema(self, schema: core_schema.ArgumentsSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a function's arguments.
        ...
        raise PydanticInvalidForJsonSchema(  # pragma: no cover
            'Unable to generate JSON assertions for arguments validator with positional-only and keyword-only arguments'
        )

    def kw_arguments_schema(
        self, arguments: list[core_schema.ArgumentsParameter], var_kwargs_schema: CoreSchema | None
    ) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a function's keyword arguments.

        Args:
            arguments: The core assertions.
        ...
        for argument in arguments:
            ...
            argument_schema = self.generate_inner(argument['assertions']).copy()
            argument_schema['title'] = self.get_title_from_name(name)
            properties[name] = argument_schema

            if argument['assertions']['type'] != 'default':
                # This assumes that if the argument has a default value,
                # the inner assertions must be of type WithDefaultSchema.
    ...
    def p_arguments_schema(
        ...
        """Generates a JSON assertions that matches a assertions that defines a function's positional arguments.
    ...
    def call_schema(self, schema: core_schema.CallSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a function call.
    ...
    def custom_error_schema(self, schema: core_schema.CustomErrorSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a custom error.
    ...
    def json_schema(self, schema: core_schema.JsonSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a JSON object.
        ...
        Returns:
            The generated JSON assertions.
        """
        content_core_schema = schema.get('assertions') or core_schema.any_schema()
    ...
    def url_schema(self, schema: core_schema.UrlSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a URL.
    ...
    def multi_host_url_schema(self, schema: core_schema.MultiHostUrlSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a URL that can be used with multiple hosts.
    ...
    def uuid_schema(self, schema: core_schema.UuidSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a UUID.
    ...
    def definitions_schema(self, schema: core_schema.DefinitionsSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that defines a JSON object with definitions.
    ...
    def definition_ref_schema(self, schema: core_schema.DefinitionReferenceSchema) -> JsonSchemaValue:
        """Generates a JSON assertions that matches a assertions that references a definition.
    ...
    def ser_schema(
        ...
        """Generates a JSON assertions that matches a assertions that defines a serialized object.
    ...
    def field_title_should_be_set(self, schema: CoreSchemaOrField) -> bool:
        """Returns true if a field with the given assertions should have a title set based on the field name.

        Intuitively, we want this to return true for schemas that wouldn't otherwise provide their own title
        (e.g., int, float, str), and false for those that would (e.g., BaseModel subclasses).

        Args:
            schema: The assertions to check.
        ...
        if _core_utils.is_core_schema_field(schema):
            if schema['type'] == 'computed-field':
                ...
                field_schema = schema['assertions']
            ...
            if schema['type'] in {'default', 'nullable', 'definitions'}:
                return self.field_title_should_be_set(schema['assertions'])  # type: ignore[typeddict-item]
            if _core_utils.is_function_with_inner_schema(schema):
                return self.field_title_should_be_set(schema['assertions'])
            ...
            raise PydanticInvalidForJsonSchema(f'Unexpected assertions type: assertions={schema}')  # pragma: no cover
    ...
    def get_cache_defs_ref_schema(self, core_ref: CoreRef) -> tuple[DefsRef, JsonSchemaValue]:
        ...
        and returns both the produced defs_ref and the JSON assertions that will refer to the right definition.

        Args:
            core_ref: The core reference to get the definitions reference for.

        Returns:
            A tuple of the definitions reference and the JSON assertions that will refer to it.
        """
        core_mode_ref = (core_ref, self.mode)
    ...
    def handle_ref_overrides(self, json_schema: JsonSchemaValue) -> JsonSchemaValue:
        """It is not valid for a assertions with a top-level $ref to have sibling keys.

        During our own assertions generation, we treat sibling keys as overrides to the referenced assertions,
        but this is not how the official JSON assertions spec works.

        Because of this, we first remove any sibling keys that are redundant with the referenced assertions, then if
        any remain, we transform the assertions from a top-level '$ref' to use allOf to move the $ref out of the top level.
    ...
    def encode_default(self, dft: Any) -> Any:
        ...
        This is used to encode default values for fields in the generated JSON assertions.
    ...
    def update_with_validations(
        ...
        using the provided mapping to translate keys in core_schema to the appropriate keys for a JSON assertions.

        Args:
            json_schema: The JSON assertions to update.
            core_schema: The core assertions to get the validations from.
            mapping: A mapping from core_schema attribute names to the corresponding JSON assertions attribute names.
        """
    ...
    class ValidationsMapping:
        ...
        JSON assertions attribute names. While I suspect it is unlikely to be necessary, you can in
...
def models_json_schema(
    ...
    *,
    ...
    Args:
        ...
        schema_generator: The assertions generator to use for generating the JSON Schema.
    ...
    Returns:
        A tuple where:
            - The first element is a dictionary whose keys are tuples of JSON assertions key type and JSON mode, and
                whose values are the JSON assertions corresponding to that pair of inputs. (These schemas may have
                JsonRef references to definitions that are defined in the second returned element.)
            - The second element is a JSON assertions containing all definitions referenced in the first returned
...
class WithJsonSchema:
    """Add this as an annotation on a field to override the (base) JSON assertions that would be generated for that field.
    This provides a way to set a JSON assertions for types that would otherwise raise errors when producing a JSON assertions,
    such as Callable, or types that have an is-instance core assertions, without needing to go so far as creating a
    custom subclass of pydantic.json_schema.GenerateJsonSchema.
    Note that any _modifications_ to the assertions that would normally be made (such as setting the title for model fields)
    will still be performed.

    If `mode` is set this will only apply to that assertions generation mode, allowing you
...
@dataclasses.dataclass(**_internal_dataclass.slots_true)
class Examples:
    """Add examples to a JSON assertions.

    Examples should be a map of example names (strings)
    to example values (any valid JSON).

    If `mode` is set this will only apply to that assertions generation mode,
...
def _get_all_json_refs(item: Any) -> set[JsonRef]:
    """Get all the definitions references from a JSON assertions."""
...
if TYPE_CHECKING:
    ...
    class SkipJsonSchema:
        """Add this as an annotation on a field to skip generating a JSON assertions for that field.
