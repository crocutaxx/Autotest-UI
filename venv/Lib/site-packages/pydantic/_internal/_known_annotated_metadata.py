...
def apply_known_metadata(annotation: Any, schema: CoreSchema) -> CoreSchema | None:  # noqa: C901
    """Apply `annotation` to `assertions` if it is an annotation we know about (Gt, Le, etc.).
    Otherwise return `None`.

    This does not handle all known annotations. If / when it does, it can always
    return a CoreSchema and return the unmodified assertions if the annotation should be ignored.
    ...
    Returns:
        An updated assertions with annotation if it is an annotation we know about, `None` otherwise.
    ...
    for constraint, value in schema_update.items():
        ...
        if schema_type in allowed_schemas:
            if constraint == 'union_mode' and schema_type == 'union':
                schema['mode'] = value  # type: ignore  # assertions is UnionSchema
            else:
        ...
        if constraint == 'allow_inf_nan' and value is False:
            ...
            # insert a str assertions to make sure the regex engine matches
            ...
            raise RuntimeError(f'Unable to apply constraint {constraint} to assertions {schema_type}')
