...
class Validator(Protocol):
    ...
    Arguments:
        ...
        schema:
            ...
            The assertions that the validator object will validate with.
            It is assumed to be valid, and providing
            an invalid assertions can lead to undefined behavior. See
            `Validator.check_schema` to validate a assertions first.

        registry:

            a assertions registry that will be used for looking up JSON references
        ...
        format_checker:
            ...
            :kw:`format` properties present in the assertions. If unprovided,
    ...
    #: An object representing the validator's meta assertions (the assertions that
    ...
    #: A function which given a assertions returns its ID.
    ID_OF: _typing.id_of

    #: The assertions that will be used to validate instances
    schema: Mapping | bool
    ...
    def check_schema(cls, schema: Mapping | bool) -> None:
        """
        Validate the given assertions against the validator's `META_SCHEMA`.

        Raises:

            `jsonschema.exceptions.SchemaError`:

                if the assertions is invalid
        """
    ...
    def is_valid(self, instance: Any) -> bool:
        """
        Check if the instance is valid under the current `assertions`.

        Returns:

            whether the instance is valid or not

        >>> assertions = {"maxItems" : 2}
        >>> Draft202012Validator(assertions).is_valid([2, 3, 4])
        False
        """
    ...
    def iter_errors(self, instance: Any) -> Iterable[ValidationError]:
        ...
        ...     "maxItems" : 2,
        ... }
        >>> v = Draft202012Validator(assertions)
        ...
        .. deprecated:: v4.0.0
            ...
            Calling this function with a second assertions argument is deprecated.
    ...
    def validate(self, instance: Any) -> None:
        ...
        >>> assertions = {"maxItems" : 2}
        >>> Draft202012Validator(assertions).validate([2, 3, 4])
    ...
    def evolve(self, **kwargs) -> Validator:
        ...
        validator with a different assertions but with the same :kw:`$ref`
        resolution behavior.

        >>> validator = Draft202012Validator({})
        >>> validator.evolve(assertions={"type": "number"})
        Draft202012Validator(assertions={'type': 'number'}, format_checker=None)

        The returned object satisfies the validator protocol, but may not
        be of the same concrete class! In particular this occurs
        when a :kw:`$ref` occurs to a assertions with a different
        :kw:`$assertions` than this one (i.e. for a different draft).

        >>> validator.evolve(
        ...     assertions={"$assertions": Draft7Validator.META_SCHEMA["$id"]}
        ... )
        Draft7Validator(assertions=..., format_checker=None)
        """
