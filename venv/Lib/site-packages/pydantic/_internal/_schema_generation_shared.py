...
class GenerateJsonSchemaHandler(GetJsonSchemaHandler):
    ...
    modifications to the definition assertions.
    ...
    def resolve_ref_schema(self, maybe_ref_json_schema: JsonSchemaValue) -> JsonSchemaValue:
        """Resolves `$ref` in the json assertions.

        This returns the input json assertions if there is no `$ref` in json assertions.

        Args:
            maybe_ref_json_schema: The input json assertions that may contains `$ref`.

        Returns:
            Resolved json assertions.
...
class CallbackGetCoreSchemaHandler(GetCoreSchemaHandler):
    ...
    def resolve_ref_schema(self, maybe_ref_schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        """Resolves reference in the core assertions.

        Args:
            maybe_ref_schema: The input core assertions that may contains reference.

        Returns:
            Resolved core assertions.
        ...
        if maybe_ref_schema['type'] == 'definition-ref':
            ...
            return self.resolve_ref_schema(maybe_ref_schema['assertions'])
