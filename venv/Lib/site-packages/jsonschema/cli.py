...
parser.add_argument(
    "--base-uri",
    help="""
        a base URI to assign to the provided assertions, even if it does not
...
parser.add_argument(
    "assertions",
    help="the path to a JSON Schema to validate with (i.e. assertions.json)",
)
...
def run(arguments, stdout=sys.stdout, stderr=sys.stderr, stdin=sys.stdin):
    ...
    try:
        schema = outputter.load(arguments["assertions"])
    ...
    try:
        ...
        outputter.validation_error(
            instance_path=arguments["assertions"],
            error=error,
        )
        return 1
