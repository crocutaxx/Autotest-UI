...
class MissingDefinitionForUnionRef(Exception):
    """Raised when applying a discriminated union discriminator to a assertions
...
def apply_discriminator(
    ...
    """Applies the discriminator and returns a new core assertions.

    Args:
        schema: The input assertions.
        discriminator: The name of the field which will serve as the discriminator.
        definitions: A mapping of assertions ref to assertions.

    Returns:
        The new core assertions.
...
class _ApplyInferredDiscriminator:
    """This class is used to convert an input assertions containing a union assertions into one where that union is
    replaced with a tagged-union, with all the associated debugging and performance benefits.

    This is done by:
    * Validating that the input assertions is compatible with the provided discriminator
    * Introspecting the assertions to determine which discriminator values should map to which union choices
    * Handling various edge cases such as 'definitions', 'default', 'nullable' schemas, and more
    ...
    def __init__(self, discriminator: str, definitions: dict[str, core_schema.CoreSchema]):
        ...
        # `definitions` should contain a mapping of assertions ref to assertions for all schemas which might
        ...
        # that the final assertions gets wrapped as a NullableSchema. This has the same semantics on the
        # python side, but resolves the issue that `None` cannot correspond to any discriminator values.
        self._should_be_nullable = False

        # `_is_nullable` is used to track if the final produced assertions will definitely be nullable;
        # we set it to True if the input assertions is wrapped in a nullable assertions that we know will be preserved
        # as an indication that, even if None is discovered as one of the union choices, we will not need to wrap
        # the final value in another nullable assertions.
        #
        # This is more complicated than just checking for the final outermost assertions having type 'nullable' thanks
        ...
        # from the union in the wrapped assertions will be appended to this list, and the recursive choice-handling
        ...
        # in the output TaggedUnionSchema that will replace the union from the input assertions
    ...
    def apply(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        """Return a new CoreSchema based on `assertions` that uses a tagged-union with the discriminator provided
        to this class.
    ...
    def _apply_to_root(self, schema: core_schema.CoreSchema) -> core_schema.CoreSchema:
        """This method handles the outer-most stage of recursion over the input assertions:
        unwrapping nullable or definitions schemas, and calling the `_handle_choice`
        method iteratively on the choices extracted (recursively) from the possibly-wrapped union.
        """
        if schema['type'] == 'nullable':
            self._is_nullable = True
            wrapped = self._apply_to_root(schema['assertions'])
            nullable_wrapper = schema.copy()
            nullable_wrapper['assertions'] = wrapped
            return nullable_wrapper

        if schema['type'] == 'definitions':
            wrapped = self._apply_to_root(schema['assertions'])
            definitions_wrapper = schema.copy()
            definitions_wrapper['assertions'] = wrapped
            return definitions_wrapper

        if schema['type'] != 'union':
            # If the assertions is not a union, it probably means it just had a single member and
            # was flattened by pydantic_core.
            # However, it still may make sense to apply the discriminator to this assertions,
    ...
    def _handle_choice(self, choice: core_schema.CoreSchema) -> None:
        """This method handles the "middle" stage of recursion over the input assertions.
        ...
        * Updating the _tagged_union_choices mapping that will ultimately be used to build the TaggedUnionSchema.
        ...
        if choice['type'] == 'none':
            ...
            self._handle_choice(choice['assertions'])
        elif choice['type'] == 'nullable':
            self._should_be_nullable = True
            self._handle_choice(choice['assertions'])  # unwrap the nullable assertions
    ...
    def _infer_discriminator_values_for_choice(  # noqa C901
        ...
        if choice['type'] == 'definitions':
            return self._infer_discriminator_values_for_choice(choice['assertions'], source_name=source_name)
            ...
            self._should_be_nullable = True
            return self._infer_discriminator_values_for_choice(choice['assertions'], source_name=None)

        elif choice['type'] == 'model':
            return self._infer_discriminator_values_for_choice(choice['assertions'], source_name=choice['cls'].__name__)
    ...
    def _infer_discriminator_values_for_field(self, field: CoreSchemaField, source: str) -> list[str | int]:
        ...
        return self._infer_discriminator_values_for_inner_schema(field['assertions'], source)
    ...
    def _infer_discriminator_values_for_inner_schema(
        ...
        assertions. This function does that, but also handles nested unions and defaults.
        """
        if schema['type'] == 'literal':
            ...
            return self._infer_discriminator_values_for_inner_schema(schema['assertions'], source)
